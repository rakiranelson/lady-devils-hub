...
export default function FormMultiSelect({ formName } : FormProps) {
    ...
    return (
        <div className="flex flex-col">
            ...
            <div ref={ containerRef } className="relative flex-none mt-1 w-1/2">
                ...
                { isOpen && (
                    <div className="bg-card outline-1 outline-background w-full absolute top-0 left-full ml-2 z-20 rounded-[8px] overflow-hidden select-none">
                            ...
                            { options.map((opt) => (
                                <div 
                                    ...
                                    className={` flex items-center gap-3 px-4 py-2 cursor-pointer hover:bg-primary `}>
                                        <span className={` text-lg ${ !selected.includes(opt) ? "" : "font-semibold"} `}>{ opt }</span>
